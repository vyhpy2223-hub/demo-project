...
async function loadPage(pagePath, title, sub, activeBtn) {
  ...
  try {
    ...
    // Init dynamic behaviors for the loaded module (if provided)
    if (typeof window.initMentorModule === "function") {
      try { window.initMentorModule(pagePath); } catch (e) { console.error(e); }
    }
